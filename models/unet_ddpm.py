...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None, y=None):
        ...
        if self.time_bias is not None:
            ...
            # out += self.time_bias(self.activation(time_emb[:,:,1]))[:, :, None, None]
            # out += self.time_bias(self.activation(time_emb[:,:,2]))[:, :, None, None]
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        # self.init_day_mlp = nn.Sequential(
        #     PositionalEmbedding(base_channels, max_period=366),
        #     nn.Linear(base_channels, base_channels),
        #     nn.SiLU(),
        #     nn.Linear(base_channels, base_channels),
        # ) 
        # self.init_hour_mlp = nn.Sequential(
        #     PositionalEmbedding(base_channels, max_period=24),
        #     nn.Linear(base_channels, base_channels),
        #     nn.SiLU(),
        #     nn.Linear(base_channels, base_channels),
        # ) 
        self.fore_time_mlp = nn.Sequential(
            PositionalEmbedding(base_channels, max_period=20),
            nn.Linear(base_channels, base_channels),
            nn.SiLU(),
            nn.Linear(base_channels, base_channels),
        ) 
        ...
        self.out_conv = nn.Conv2d(base_channels, output_channels, 3, padding=1)
    
    def forward(self, x, t=None,time=None, y=None):
        ip = self.initial_pad
        if ip != 0:
            x = F.pad(x, (ip,) * 4)

        if self.fore_time_mlp is not None:
            if t is None:
                raise ValueError("time conditioning was specified but tim is not passed")
            # init_day_emb = self.init_day_mlp(time[:,0])
            # init_hour_emb = self.init_hour_mlp(time[:,0])
            # fore_time_emb = self.fore_time_mlp(t)
            # time_emb=torch.stack([init_day_emb,init_hour_emb,fore_time_emb],dim=2)
            time_emb = self.fore_time_mlp(t)
        else:
            time_emb = None
...
if __name__=='__main__':
    ...
    model=UNet(img_channels=70*2,
        base_channels=64,output_channels=5).to(DEVICE)
    x=torch.randn(16,2*70 ,161,161).to(DEVICE)
    y=model(x,time=torch.tensor(range(10,26,1)).float().to(DEVICE))
    print(y.shape)