...
class U2NET(nn.Module):
    def __init__(self, cfgs, out_ch,out_seq=1):
    ...
    def forward(self, x):
        ...
        def fuse():
            ...
            if self.out_seq>1:
                B,C,H,W=x.shape
                x=x.contiguous().view(B,self.out_seq,-1,H,W)
            return x
...
def U2NET_full(in_ch=20,out_ch=20,out_seq=1):
    full = {
    ...
    return U2NET(cfgs=full, out_ch=out_ch,out_seq=out_seq)


def U2NET_lite(in_ch=20,out_ch=20,out_seq=1):
    lite = {
        # cfgs for building RSUs and sides
        # {stage : [name, (height(L), in_ch, mid_ch, out_ch, dilated), side]}
    ...
    return U2NET(cfgs=lite, out_ch=out_ch,out_seq=out_seq)
...
if __name__=='__main__':
    ...
    print('start')
    model=U2NET_full(in_ch=2*10,out_ch=20)
    if torch.cuda.device_count() > 1:
            model = nn.DataParallel(model)
    model=model.to(device)
    from tqdm import tqdm
    x=torch.randn(12,20,480,560).to(device)
